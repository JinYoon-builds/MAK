...
카메라(사람 존재 + 입술 움직임)와 마이크(음성)를 시간적으로 융합해 "지금 앞사람이 말하는가"를 판단합니다. 입술 모션은 머리 자세에 강건한 MediaPipe 블렌드셰이프의 **벌어짐 진폭**으로 잡고, 하드 게이트가 아니라 **confidence 분기(진행/확인/폐기)** 로 씁니다. 보정/디버그는 `presence-vad.html` 벤치에서 합니다.
